...
GO_SERVICE_URL = os.getenv("GO_SERVICE_URL", "http://go-service:8001")
RUST_SERVICE_URL = os.getenv("RUST_SERVICE_URL", "http://rust-service:8002")
...
def create_workout(
    ...
    for order, exercise_input in enumerate(workout.exercises):
        exercise = (
            db.query(Exercise).filter(Exercise.name == exercise_input.name).first()
        )

        if not exercise:
            raise HTTPException(
                status_code=404, detail=f"種目 '{exercise_input.name}' が見つかりません"
            )
...
def get_workout_by_date(
    ...
    workout = (
        db.query(Workout)
        .filter(Workout.date == date, Workout.user_id == current_user.id)
        .first()
    )

    if not workout:
        raise HTTPException(
            status_code=404, detail=f"{date}の記録が見つかりませんでした"
        )
...
def delete_workout_by_date(
    ...
    try:
        db.query(Set).filter(
            Set.workout_exercise_id.in_(
                db.query(WorkoutExercise.id).filter(
        ...
        db.query(Workout).filter(Workout.id.in_(workout_ids)).delete(
...
def add_exercises_to_workout(
    ...
    max_order = (
        db.query(func.max(WorkoutExercise.order))
...
def delete_exercise_by_id(
    ...
    if used:
        raise HTTPException(
            status_code=400,
            detail="この種目はワークアウト記録で使用中のため削除できません",
        )
...
def delete_exercise_id(
    ...
    try:
        db.query(Set).filter(Set.workout_exercise_id == workout_exercise_id).delete(
            synchronize_session=False
        )
...
def add_set_to_workout_exercise(
    workout_exercise_id: int, set_data: SetCreate, db: Session = Depends(get_db)
):
    workout_exercise = (
        db.query(WorkoutExercise)
...
async def get_personal_records(current_user: User = Depends(get_current_user)):
    async with httpx.AsyncClient() as client:
        response = await client.get(
            f"{RUST_SERVICE_URL}/internal/stats/personal_records",
...
def get_frequency(
    ...
    this_week = (
        db.query(func.count(Workout.id))
        .filter(Workout.user_id == current_user.id, Workout.date >= this_week_start)
        .scalar()
    )
    ...
    if first_date and total > 0:
        weeks = max(
            (today.date() - datetime.strptime(first_date, "%Y-%m-%d").date()).days / 7,
            1,
        )
